...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        self.model = Transformer(n_feat=feature_size, n_channel=seq_length, n_layer_enc=n_layer_enc, n_layer_dec=n_layer_dec,
                                 ...
                                 max_len=seq_length, n_embd=d_model, conv_params=[kernel_size, padding_size],num_classes = num_classes, **kwargs)
    ...
    def output(self, x, t, padding_masks=None, labels = None):
        trend, season = self.model(x, t, padding_masks=padding_masks, labels = labels)
        model_output = trend + season
        return model_output

    def model_predictions(self, x, t, clip_x_start=False, padding_masks=None, labels = None):
        if padding_masks is None:
            padding_masks = torch.ones(x.shape[0], self.seq_length, dtype=bool, device=x.device)

        maybe_clip = partial(torch.clamp, min=-1., max=1.) if clip_x_start else identity
        x_start = self.output(x, t, padding_masks, labels = labels)
        x_start = maybe_clip(x_start)
        pred_noise = self.predict_noise_from_start(x, t, x_start)
        return pred_noise, x_start

    def p_mean_variance(self, x, t, clip_denoised=True, labels = None):
        _, x_start = self.model_predictions(x, t, labels = labels)
        if clip_denoised:
            x_start.clamp_(-1., 1.)
    ...
    def p_sample(self, x, t: int, clip_denoised=True, cond_fn=None, model_kwargs=None, labels = None):
        b, *_, device = *x.shape, self.betas.device
        batched_times = torch.full((x.shape[0],), t, device=x.device, dtype=torch.long)
        model_mean, _, model_log_variance, x_start = \
            self.p_mean_variance(x=x, t=batched_times, clip_denoised=clip_denoised, labels = labels)
    ...
    def sample(self, shape, labels = None):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)),
            ...
            img, _ = self.p_sample(img, t, labels = labels)
        return img

    @torch.no_grad()
    def fast_sample(self, shape, clip_denoised=True, labels = None):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, clip_x_start=clip_denoised, labels = labels)
    ...
    def generate_mts(self, batch_size=16, model_kwargs=None, cond_fn=None, labels = None):
        feature_size, seq_length = self.feature_size, self.seq_length
        if cond_fn is not None:
            sample_fn = self.fast_sample_cond if self.fast_sampling else self.sample_cond
            return sample_fn((batch_size, seq_length, feature_size), model_kwargs=model_kwargs, cond_fn=cond_fn, labels = labels)
        sample_fn = self.fast_sample if self.fast_sampling else self.sample
        return sample_fn((batch_size, seq_length, feature_size), labels = labels)
