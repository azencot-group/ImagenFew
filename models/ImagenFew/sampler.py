...
class DiffusionProcess():
    ...
    def sample(self, latents, mask, class_labels=None):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.net(x_hat*mask, t_hat, class_labels).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < self.num_steps - 1:
                denoised = self.net(x_next*mask, t_next, class_labels).to(torch.float64)
    ...
    @torch.no_grad()
    def sampling(self, mask, sampling_number=16, impute=False, xT=None,class_labels=None):
        if xT is None:
            xT = torch.randn([sampling_number, *self.shape]).to(device=self.device)
        return self.sample(xT, mask, class_labels = class_labels)
